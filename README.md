...
- 文件夹名称清理已拆分为独立工具：普通文本删除、智能规则删除、高级正则删除、替换、添加、首尾空格和无用内容推荐。
- 不会正则也可以使用智能规则删除，例如删除`048 ARTGRAVIA...`里的开头编号，或删除`DJAWA Photo Vol 0216`这类Photo Vol编号。
- 高级正则提供示例按钮和小抄说明，适合需要自定义匹配规则的情况。
- 所有操作先扫描、预览、确认，再执行。
...
文件夹名称清理区每个卡片都有独立执行按钮，只会执行当前卡片对应的规则。
...
清理工具采用先预览、再执行的方式。推荐优先使用“智能规则删除”和“无用内容推荐”：

- 删除开头编号：`[Son Ye-Eun 손예은] - 048 ARTGRAVIA_VOL252 #1 [23P 33MB]`会变成`[Son Ye-Eun 손예은] - ARTGRAVIA_VOL252 #1 [23P 33MB]`。
- 删除Photo Vol编号：`[Son Ye-Eun 손예은] - DJAWA Photo Vol 0216 - Staycation #5 [149P 2.99GB]`会变成`[Son Ye-Eun 손예은] - Staycation #5 [149P 2.99GB]`。
- 高级正则示例：`DJAWA Photo Vol \d+`匹配Photo Vol编号，`^\d{2,4}\s+`匹配开头2到4位数字。

无用内容推荐只给出候选项，不会自动删除。用户勾选候选项并确认预览后才会执行。
